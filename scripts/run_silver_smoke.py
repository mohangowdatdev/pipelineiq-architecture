"""Smoke-test a Silver notebook against the live workspace.

Steps:
1. Upload `notebooks/silver/build_silver_{entity}.py` into the workspace.
2. Submit a one-time job that runs it.
3. Poll until the run finishes; print result + any error.

Auth: AAD via DefaultAzureCredential — uses your `az login` session.

Usage:
    .venv/bin/python scripts/run_silver_smoke.py                # default entity=orders
...
LOCAL_NOTEBOOK_DIR = Path(__file__).resolve().parent.parent / "notebooks/silver"
WS_NOTEBOOK_DIR = "/Shared/pipelineiq/silver"


def upload_notebook(w: WorkspaceClient, entity: str) -> str:
    local = LOCAL_NOTEBOOK_DIR / f"build_silver_{entity}.py"
    if not local.exists():
        sys.exit(f"Local notebook not found: {local}")

    ws_path = f"{WS_NOTEBOOK_DIR}/build_silver_{entity}"
...
def submit_run(w: WorkspaceClient, entity: str, ws_path: str) -> int:
    ...
    run = w.jobs.submit(
        run_name=f"silver-smoke-{entity}",
        tasks=[
            jobs.SubmitTask(
                task_key="silver",
...
def main() -> int:
    ...
    ap.add_argument("--entity", default="orders", help="Silver entity to build (default: orders).")
    args = ap.parse_args()
