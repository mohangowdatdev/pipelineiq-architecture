...
| 6.11 | Cutover: production fire path is ADF, not laptop script | **Pending (unblocked, awaiting go)** | — | `az datafactory trigger start -n trg_daily_0040`; demote `scripts/export_velora_to_landing.py` to recovery | Chunk 2. 6.5 now Done + full smoke green, so this is unblocked. **Held for explicit go:** before the nightly trigger, weigh the bronze re-read-all-and-append growth (bronze gains a full landing snapshot per night; silver/gold stay exact via MERGE) — see CLAUDE.md carry-over. `trg_daily_0040` deploys Stopped. |
