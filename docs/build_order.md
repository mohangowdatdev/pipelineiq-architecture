...
| 9.4i | Gold chunk 2 — dim_product (SCD-2) + dim_sales_rep (SCD-2) + dim_territory (synthesized) | Pending | — | TBD per CLAUDE.md "Medallion chunk plan" | Next session (S10). Full specs in SCHEMA.md `gold.dim_product` / `gold.dim_sales_rep` / `gold.dim_territory`. Closest existing notebook to copy: `build_gold_dim_customer.py`. |
...
| 9.4 | Bronze → Silver → Gold full run | In progress | — | Databricks Job | End-to-end Phase 2 exit. Bronze 12/12 (10 main + 2 static seeds via 9.4g); Silver 7/10; Gold 6/12 (1 SCD-2 dim from S8 + 5 static dims via 9.4h). Remaining: 3 Silver + 6 Gold (3 SCD-2 dims + 3 facts) per CLAUDE.md "Medallion chunk plan (S10 → S13)". |
