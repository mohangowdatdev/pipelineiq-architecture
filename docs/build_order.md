...
| # | Item | Status | Date | Path / command | Notes |
|---|---|---|---|---|---|
| 6.1 | Data Factory `pipelineiq-adf-dev` | In progress (code ✅, apply pending) | 2026-06-04 | `pipelineiq-iac/core/adf/` | S18: module written — system-assigned MI, Git disabled (bicep-first), + RBAC (Storage Blob Data Contributor on ADLS, KV Secrets User, Contributor on Databricks workspace). Wired into `clients/velora/main.tf` + outputs. `terraform validate` => Success. **Not yet applied** — S18 apply blocked on off-VPN Azure auth (token refresh to login.microsoftonline.com timing out). Resume: prime az token → `terraform plan -out=tfplan -target=module.adf` → apply. |
| 6.2 | Linked services: Azure SQL, ADLS, Key Vault, Databricks | In progress (code ✅, deploy pending) | 2026-06-04 | `pipelineiq-iac/bicep/adf/*.bicep` | S18: 4 Bicep written — KV (MI), Azure SQL (KV-referenced `sql-connection-string`), ADLS Gen2 (MI), Databricks (**MSI**, DECISIONS #74). `az bicep build` => valid ARM. Deploy via `scripts/deploy_adf.sh` after 6.1 applies. |
| 6.3 | Parameterised datasets (Velora) | In progress (code ✅, deploy pending) | 2026-06-04 | `pipelineiq-iac/bicep/adf/dataset_*.bicep` | S18: SQL source + ADLS Parquet sink, both parameterised by `{schema, table, watermark_column, load_type}` (sink adds computed `folder_path`) for all 12 `entity_registry` rows. Compiles clean. Deploy with 6.2. |
