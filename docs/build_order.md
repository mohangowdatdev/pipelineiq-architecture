...
| 6.4 | Copy pipeline: Azure SQL → landing | **Done** | 2026-06-07 | `PipelineIQ-IaC` `45b56d7` (`pipeline_master_copy.bicep`) | Chunk 2, faithful reproduction (DECISIONS #76). **Copy smoke GREEN** (run `0e323e94`, 2026-06-04): ForEach over `GET /entities`, all 12 entities → `landing/{source_table}/[date=|full]/` driven by `partition_date_column`; `landing/orders/date=2026-06-04/` + `landing/customers/full/` verified. `ClearTarget`→`Copy` ("Completed" dep tolerates first-run missing folder). |
| 6.5 | Databricks notebook activities: Bronze / Silver / Gold | **Done** | 2026-06-09 | `core/medallion_workflow/` (TF job) + `RunMedallion`→run-now in `pipeline_master_copy.bicep` + `notebooks/orchestrate_medallion.py` | Chunk 2, DECISIONS #77 → **#79/#80/#81 (S20)**. UC blocker (#78) resolved via a TF-defined `SINGLE_USER` Databricks job (Option 1b, #79) triggered by ADF `jobs/run-now` + Until-poll + Assert; bronze enforces ADF-canonical types for mixed-writer landing (#80); gold DAG orders `static_dims` before `dim_territory` (#81). Medallion dropped+rebuilt clean-slate to ADF types (bronze 12/12, silver 10/10, gold 12/12; facts reconcile silver exactly; 0 collisions; 0 FK orphans). **End-to-end smoke GREEN** (run `215da040`, 2026-06-04, 100/100 activities). |
| 6.6 | Diagnostic settings → Log Analytics | **Done** | 2026-06-07 | `PipelineIQ-IaC` `core/adf/main.tf` | Chunk 2. `azurerm_monitor_diagnostic_setting` on the factory → `pipelineiq-logs-dev` (`PipelineRuns`/`ActivityRuns`/`TriggerRuns` + AllMetrics) applied. Required for Phase 3 failure detection. |
| 6.7 | `pipeline.entity_registry` CONSUMED by ADF master pipeline | **Done** | 2026-06-07 | `GET /entities` + `pl_master_copy` ForEach | Chunk 2, DECISIONS #75. **Metadata-driven is now FACT** — smoke `0e323e94` showed `GetEntities`→`ForEach`(12 registry rows) live; `get_entities` deployed + returns `partition_date_column`. |
| 6.8 | `pipeline.watermarks` read/written via Function REST | **Done** | 2026-06-07 | ADF `CommitWatermark` → `POST /watermarks/{entity}/commit` | Chunk 2. **Proven:** `velora_oms.orders` watermark advanced to 2026-06-04 via the dotted-route `AzureFunctionActivity`. Per-copy commit inside ForEach (DECISIONS #75 deviation). |
| 6.9 | `pipeline.file_registry` written on every landed Parquet | **Done** | 2026-06-07 | ADF `RegisterFile` → `POST /files/register` | Chunk 2. `RegisterFile` succeeded for all 12 entities in smoke `0e323e94` (file_path, source_entity, landed_at, row_count=`@...rowsCopied`, pipeline_run_id). |
| 6.10 | `pipeline.pipeline_exec_log` written on run start + end | **Done** | 2026-06-07 | ADF `LogRunStart`/`LogRunEnd` → `POST /runs/start` + `POST /runs/{run_id}/end` | Chunk 2. Master run_id `@pipeline().RunId` + 12 per-entity `<RunId>:<source_table>`; `LogEntityEnd` succeeded ×12. Master closed `failed` (medallion leg). Direct row count pending VPN Postgres check. |
| 6.11 | Cutover: production fire path is ADF, not laptop script | **Done** | 2026-06-10 | `PipelineIQ-IaC` `2fb071d` (`trg_daily_0100` @ 01:00 UTC + STARTED); scaffold `⛔ DO NOT RUN` (S21) | Chunk 2. Cutover committed S20-cont (`2fb071d`, trigger renamed `trg_daily_0040`→`trg_daily_0100` @ 01:00 UTC for a safer buffer after the 00:35 inventory Job, and **Started**). `scripts/export_velora_to_landing.py` demoted to `⛔ DO NOT RUN` (S21) — ADF is sole landing writer; gap recovery is a manual `pl_master_copy` fire with `run_date`. Bronze-growth concern **resolved** by DECISIONS #82 (bronze now incremental/idempotent — one `_load_date` partition per night, not a full re-append). Full ADF→incremental-medallion chain validated end-to-end (manual fire run_date=2026-06-08, Succeeded, idempotent). **Op follow-up (not part of this row):** confirm the first autonomous fire — trigger `startTime` anchor is `2026-06-10T01:00:00Z`, so first occurrence is 06-11 01:00 UTC. |
...
|---|---|---|---|---|---|
| 9.1 | Generator dry-run against provisioned Azure SQL | **Done** | 2026-06-05 | `generator/catalogue.py::to_live_shape` + `generator/main.py` | S18: fixed the dry-run-on-unseeded-DB bug. New `catalogue.to_live_shape()` re-projects the in-memory built catalogue into the `load_from_db` shape; `run()` uses it on the `dry_run + catalogue-built` path instead of reading the (empty) DB. Unit-tested: 4,200 products, all priced, correct columns. (Seeded-DB dry-run already worked; bug was unseeded-only.) |
