...
| 6.5 | Databricks notebook activities: Bronze / Silver / Gold | **Blocked** | 2026-06-07 | `RunMedallion` activity (deployed) + `notebooks/orchestrate_medallion.py` | Chunk 2, DECISIONS #77 + **#78**. Orchestrator notebook + `RunMedallion` (DatabricksNotebook activity via `ls_databricks`) deployed. **Smoke FAILED:** ADF cluster lacks `data_security_mode=SINGLE_USER` → no Unity Catalog access → bronze UC write faults (MSI auth worked). Fix = UC-cluster follow-up (`policyId` on `ls_databricks`, or TF-defined UC job). |
...
| 6.11 | Cutover: production fire path is ADF, not laptop script | Pending | — | Decommission `scripts/export_velora_to_landing.py` from prod | Chunk 2. **Blocked on 6.5** — enable `trg_daily_0040` (00:40 UTC, deploys Stopped) only after the **full** smoke (incl. medallion) is green. Keep the script as recovery. |
