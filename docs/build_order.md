...
| 6.4 | Copy pipeline: Azure SQL → landing | In progress | 2026-06-07 | Bicep `pipeline_master_copy.bicep` (PipelineIQ-IaC, pending author) | Chunk 2. Faithful reproduction (DECISIONS #76): ForEach over `GET /entities`; per-entity Copy emits to `landing/{source_table}/[date={run_date}|full]/` driven by `partition_date_column`. **Architecture-repo prep landed** (`partition_date_column` in `bootstrap_postgres.sql` + `SCHEMA.md`). Bicep pipeline + apply + smoke pending (IaC repo not in this checkout). |
| 6.5 | Databricks notebook activities: Bronze / Silver / Gold | In progress | 2026-06-07 | `notebooks/orchestrate_medallion.py` + `RunMedallion` activity (Bicep, pending) | Chunk 2, DECISIONS #77. **Done:** single `orchestrate_medallion` notebook (bronze 10 → silver 10 → gold 8-DAG, sequential `dbutils.notebook.run`, reuses `catchup_medallion.py` lists) + `--upload-orchestrator` deploy path. **Pending:** ADF `DatabricksNotebookActivity` over `ls_databricks` + workspace upload + smoke. Supersedes the original "separate chained activities" design. |
| 6.6 | Diagnostic settings → Log Analytics | Pending | — | Terraform `core/adf/main.tf` (PipelineIQ-IaC) | Chunk 2. `azurerm_monitor_diagnostic_setting` on the factory → `pipelineiq-logs-dev` (`PipelineRuns`/`ActivityRuns`/`TriggerRuns` + AllMetrics). Required for Phase 3 failure detection. |
| 6.7 | `pipeline.entity_registry` CONSUMED by ADF master pipeline | In progress | 2026-06-07 | `GET /entities` (`functions/function_app.py`) + Bicep ForEach (pending) | Chunk 2, DECISIONS #75. **Done:** `get_entities` endpoint (returns active rows incl. `partition_date_column`, ordered by priority). **Pending:** Function deploy + ADF ForEach that consumes it. "Metadata-driven" flips slide→fact when the ForEach lands. |
| 6.8 | `pipeline.watermarks` read/written via Function REST | In progress | 2026-06-07 | ADF `CommitWatermark` Web Activity → `POST /watermarks/{entity}/commit` | Chunk 2. **Deviation (DECISIONS #75):** watermark committed per-copy inside the ForEach (before medallion), not strict end-to-end — acceptable under faithful reproduction. Endpoint already live (4.8); ADF consumer pending. |
| 6.9 | `pipeline.file_registry` written on every landed Parquet | In progress | 2026-06-07 | ADF `RegisterFile` Web Activity → `POST /files/register` | Chunk 2. Audit trail: file_path, source_entity, landed_at, row_count=`@...rowsCopied`, pipeline_run_id. Endpoint live (4.8); ADF consumer pending. |
| 6.10 | `pipeline.pipeline_exec_log` written on run start + end | In progress | 2026-06-07 | ADF `LogRunStart`/`LogRunEnd` Web Activities → `POST /runs/start` + `POST /runs/{run_id}/end` | Chunk 2. Pipeline-level run_id = `@pipeline().RunId`; per-entity run_id = `<RunId>:<source_table>` (distinct so `log_run_end`'s `end_time IS NULL` match is unambiguous). Every `runs/start` must include `pipeline_name`. Endpoint live (4.8); ADF consumer pending. |
| 6.11 | Cutover: production fire path is ADF, not laptop script | Pending | — | Decommission `scripts/export_velora_to_landing.py` from prod | Chunk 2. Keep the script as a manual-recovery escape hatch. Enable `trg_daily_0040` (00:40 UTC, created Stopped) only after a clean copy smoke. |
