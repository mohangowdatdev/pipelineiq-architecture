...
| main.py | Orchestrator. Calls all modules in order. Writes orders/lines/status/dim-changes in a single transaction. **Inventory write removed S14 (DECISIONS #71); `_write_inventory_snapshot` + the laptop recovery scripts fully retired S18** — inventory recovery is `scripts/run_inventory_smoke.py` (Databricks). Supports --dry-run (fixed S18, build_order 9.1 — uses the in-memory catalogue on an unseeded DB), --date, --failure, --seed CLI flags. Azure Function entry point. |
