...
| **PostgreSQL `pipeline.*` schema** (control plane) | ⚠️ **Partially consumed (S16)** | First consumer landed: 5 Function REST endpoints (`functions/function_app.py`, S16/DECISIONS #72) read `watermarks`, write `watermarks` / `file_registry` / `pipeline_exec_log`. `pipeline_exec_log` + `file_registry` have their first-ever rows (smoke seeded log_id=1, file_id=1). **Still missing:** ADF as the second consumer — bronze/silver/gold notebooks still hardcode their entity. `process_queue` remains 0 rows (ADF will write it). Full activation = S17 (ADF chunk 1) + S18 (ADF master pipeline). |
...
| Function REST API (`get_watermark`, `commit_watermark`, `register_file`, `log_run_start`, `log_run_end`) | ✅ **Live (S16)** | All 5 endpoints in `functions/function_app.py` (V2 single-file model, DECISIONS #72). psycopg3 + connection pool, function-key auth, `POSTGRES_URL` via KV reference. Smoke green: all 200/201, `pipeline_exec_log` + `file_registry` first rows landed. Next consumer = ADF (S17). |
...
**The honest read (post-S16):** the medallion is solid. The Postgres control plane is no longer architecturally orphaned — the 5 Function REST endpoints (`functions/function_app.py`) are the first real consumer, and `pipeline_exec_log` + `file_registry` have their first rows. **But the architecturally interesting consumer (ADF, reading `entity_registry` + advancing watermarks per copy iteration) is still pending — that's S17.** "Metadata-driven" goes from "slide" → "fact" when ADF lands; today it's "live as an API surface but no one is calling it yet."
...
| functions/ | **Migrated V1 → V2 in S16 (DECISIONS #72). Function App now hosts 1 timer trigger (generator) + 5 HTTP endpoints (pipeline.* control plane) in a single `functions/function_app.py`.** | All historical S5-S14 module behaviour preserved (cold-start retry on 40613/HYT00 per DECISIONS #61; OTel SDK + LA `AppTraces` per #66; timer schedule "0 0 0 31 2 *" = never auto, Logic App admin-invokes per #69; inventory write moved to Databricks per #71). **S16 (DECISIONS #72):** Python V1 (per-function folders) → V2 (`@app.route` + `@app.timer_trigger` decorators in one file at deploy root). 5 new HTTP endpoints (`get_watermark`, `commit_watermark`, `register_file`, `log_run_start`, `log_run_end`) — function-key auth, psycopg3 + `psycopg_pool.ConnectionPool`, `POSTGRES_URL` env var via KV reference (`@Microsoft.KeyVault(VaultName=pipelineiq-kv-dev;SecretName=postgres-connection-string)`). The legacy V1 `generator/function.json` is now dead code (V2 host ignores it); the generator is invoked via the V2 wrapper that delegates to `generator.main:main`. **S16 (DECISIONS #73):** `pipeline.pipeline_exec_log` no longer append-only — `log_run_start` INSERTs, `log_run_end` UPDATEs the matching open row (matched on `run_id` AND `end_time IS NULL`). Deploy: `bash scripts/deploy_function.sh` (updated S16 to stage `function_app.py` + add psycopg deps to slim requirements). **S17 hygiene batch (deployed):** `functionTimeout` 30m → 5m (`host.json`); `logging.getLogger("generator").propagate = False` after `configure_azure_monitor` (dedupe AppTraces); dead V1 `generator/function.json` deleted; `POSTGRES_URL` adopted into IaC (`core/functions/main.tf`, no longer `az`-CLI-only drift). All 6 functions re-registered green, endpoint smoke healthy. |
...
- **Tier 6 ADF — chunk 2 END-TO-END SMOKE GREEN (S20); only cutover + `docs/pipeline.md` remain.**
  `RunMedallion` (DECISIONS #78) is **resolved** — fixed via a Terraform-defined Databricks
  job, not a `policyId` (DECISIONS #79, Option 1b): `core/medallion_workflow/` runs
  `orchestrate_medallion` on a `SINGLE_USER` cluster (created+run-as `mohan.gowda` via the
  azure-cli provider → UC access); ADF `pl_master_copy`'s `RunMedallion` is now
  `StartMedallion` (Web `jobs/run-now`, MSI) → `PollMedallion` (Until, run state latched to
  `medallion_state_json`) → `AssertMedallion` (`contains('"result_state":"SUCCESS"')`). Two
  follow-on fixes en route: bronze enforces ADF-canonical types for mixed-writer landing
  (DECISIONS #80 — native read + Catalyst cast, INT→BIGINT) and the gold DAG orders
  `static_dims` before `dim_territory` (DECISIONS #81); the medallion was **dropped +
  rebuilt clean-slate** to ADF types (bronze 12/12, silver 10/10, gold 12/12; facts
  reconcile silver exactly, 0 collisions, 0 FK orphans). All 10 Function activities got a
  `retry:4 @30s` policy (Flex cold-start 503 hardening). **End-to-end smoke green** (run
...
- ~~Cosmetic: each user log appears twice in `AppTraces`~~ **Fixed (S17).**
  `logging.getLogger("generator").propagate = False` added right after
  `configure_azure_monitor()` in `generator/main.py`; deployed in the S17
  hygiene batch alongside the 5-min `functionTimeout` + dead-`function.json`
  removal.
