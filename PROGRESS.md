...
| Source DB (`velora_oms`) | ✅ **34 days continuous** | 2026-04-27 → 2026-06-03. Product catalogue grew 4,205 → 4,214 SKUs on 2026-06-01 (inventory snapshots correctly 189,630/day after). Auto-pause delay 120 min (IaC says 60 — next apply realigns). |
| Landing (ADLS Parquet) | ✅ Up to date through 2026-06-03 | by-date partitions for orders + inventory; full snapshot per other entity. |
| Bronze (Delta) | ✅ 12/12 tables through 2026-06-03 | Re-ingested 6/03 in S18 via multi-task Job (after S17's 5/29→6/02). |
| Silver (Delta) | ✅ 10/10 tables through 2026-06-03 | 100% DQ pass. `inventory_snapshot` **7,190,640**; `order_lines` 48,392; `order_status_log` 38,620; `customers` 948; `customer_addresses` 948; `product_pricing` 4,249. |
| Gold dims | ✅ 9/9 dims, **0 SCD-2 collisions** | `dim_customer` 1,047 (948 current + 99 SCD-2 versions). `dim_product` 4,249 (4,214 current + 35 historical versions). `dim_sales_rep` 30. S13 bulletproof fix still holding (S18 verify: 1,047 == distinct SK, 0 collisions). |
...
**Backlog batch (2026-06-05, same session — 3 non-ADF items the user asked to clear):**
- **Medallion catch-up 6/03** (the one new complete source day; 6/05 fire writing 6/04 hadn't landed yet). Firewall already current. `audit_fires` confirmed 6/03 = 324 orders / 1,230 lines / 189,630 inventory (full grid, 0 dupes). Export 287,465 rows → bronze (10/10) → silver (10/10) → gold (8/8), all multi-task Jobs green. Verify reconciles **exact**: `fact_order_line` **48,392** == silver (+1,230 = 6/03); `fact_inventory_daily` **7,190,640** == silver (+189,630); `dim_customer` **1,047 == distinct SK, 0 collisions, 0 missing-current**; `dim_product` 4,249 / 0 collisions; `fact_daily_channel_revenue` 13,978; 0 FK orphans; static dims all expected. (Needed two verify passes — the 2X-Small warehouse hit its 5-min per-query timeout on one heavy count each run; every number resolved clean across the two.) **Medallion now current through 2026-06-03 (34 days).**
- **Retired the laptop inventory-write path (commit `76c26c5`):** deleted `scripts/inventory_only.py` + `scripts/recover_inventory_batch.sh` + the now-dead `generator/main.py::_write_inventory_snapshot` (612→498 lines). Recovery is now `scripts/run_inventory_smoke.py --date D --force` (Databricks). Docs repointed (CLAUDE.md, forward_plan, data_generation, this file).
- **Fixed `--dry-run` (build_order 9.1, commit `76c26c5`):** new `catalogue.to_live_shape()` re-projects the in-memory built catalogue into the `load_from_db` shape; `main.run()` uses it on the `dry_run + unseeded` path (was failing with `product_pool is empty`). Unit-tested: 4,200 products, all priced, exact columns. Bug was unseeded-only; seeded-DB dry-run already worked.
